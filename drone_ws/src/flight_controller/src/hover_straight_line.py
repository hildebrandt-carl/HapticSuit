    ...
    def _mainloop(self):
        ...
        while not self._quit:
            ...
            if z_pos >= self.hover_height:
                ...
                # If we are moving forward start increasing the x_pos until you reach 1
                if moving_forward:
                    x_pos += 0.2 / self.rate
                    x_pos = min(x_pos, 1)

                # If we are moving backwards start decreasing the x_pos until you reach -1
                elif not moving_forward:
                    x_pos -= 0.2 / self.rate
                    x_pos = max(x_pos, -1)

                # Check if we have reached the current position
                if x_pos >= 1:
